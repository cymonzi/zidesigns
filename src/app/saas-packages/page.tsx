...
const SAAS_PACKAGES: SaasPackage[] = [
  {
    id: "saas-mvp",
    number: "01",
    name: "SaaS MVP",
...
function SaasPackagesContent() {
  ...
  const handleContinue = () => {
    ...
    const pkg = SAAS_PACKAGES.find((p) => p.id === selectedPackage)
    if (pkg) {
      router.push(`/start-project?service=${encodeURIComponent(pkg.name)}&price=${encodeURIComponent(pkg.displayPrice)}&category=Development&phase=2&fromPackage=saas`)
    }
  }
  ...
  useEffect(() => {
    ...
    if (!svc) return
    const match = SAAS_PACKAGES.find((p) => p.name.toLowerCase() === svc.toLowerCase())
    if (match) setSelectedPackage(match.id)
  }, [searchParams])

  const selectedPkg = SAAS_PACKAGES.find((p) => p.id === selectedPackage)
      ...
      <main className="flex-1 pt-[var(--nav-height)] overflow-hidden">
        <div className="h-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6 lg:py-8 flex flex-col">
          ...
          <div className="relative pb-4 sm:pb-6 text-center flex-shrink-0">
            ...
            >
              Build your <span className="text-gradient">platform</span>
            </motion.h1>
            ...
            >
              Select a package that matches your ambition. We'll scope the details together.
            </motion.p>
          </div>
                ...
                <div className="overflow-y-auto pr-1 sm:pr-2 custom-scrollbar flex-1">
                  <div className="space-y-2.5 sm:space-y-3 md:space-y-4 pb-4">
                    ...
                    <motion.div
                      initial={{ opacity: 0, y: 10 }}
                      animate={{ opacity: 1, y: 0 }}
                      transition={{ delay: SAAS_PACKAGES.length * 0.08, duration: 0.3 }}
                      className="rounded-xl sm:rounded-2xl border border-base/60 bg-surface/50 p-3 sm:p-4 md:p-5"
                    >
                      <p className="text-xs sm:text-sm text-muted-foreground leading-relaxed">
                        <span className="font-semibold text-foreground">Every platform is unique.</span>
                        <br />
                        We'll refine features, timelines, and pricing based on your specific needs.
                      </p>
                    </motion.div>
                  </div>
                </div>
