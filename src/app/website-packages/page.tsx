...
function WebsitePackagesContent() {
  ...
  const handleContinue = () => {
    ...
    if (pkg) {
      router.push(`/start-project?service=${encodeURIComponent(pkg.name)}&price=${encodeURIComponent(pkg.displayPrice)}&category=Development&phase=2&fromPackage=website`)
    }
  }
  ...
  useEffect(() => {
    ...
    if (!svc) return
    const match = WEBSITE_PACKAGES.find((p) => p.name.toLowerCase() === svc.toLowerCase())
    if (match) setSelectedPackage(match.id)
  }, [searchParams])

  const selectedPkg = WEBSITE_PACKAGES.find((p) => p.id === selectedPackage)
                ...
                <div className="overflow-y-auto pr-1 sm:pr-2 custom-scrollbar flex-1">
                  <div className="space-y-2.5 sm:space-y-3 md:space-y-4 pb-4">
                    ...
                    <motion.div
                      initial={{ opacity: 0, y: 10 }}
                      animate={{ opacity: 1, y: 0 }}
                      transition={{ delay: WEBSITE_PACKAGES.length * 0.08, duration: 0.3 }}
                      className="rounded-2xl border border-base/60 bg-surface/50 p-5"
                    >
                      <p className="text-sm text-muted-foreground leading-relaxed">
                        <span className="font-semibold text-foreground">Need something more advanced?</span>
                        <br />
                        SaaS products, payments, complex integrations, and custom functionality are quoted based on scope.
                      </p>
                    </motion.div>
                  </div>
                </div>
